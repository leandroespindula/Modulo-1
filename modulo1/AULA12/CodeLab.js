...
//PARA SOMAR DOIS NUMEROS E RETORNAR O RESULTADO.
function soma(x,y){
    let soma;
    soma = x + y;
...
//CONTAR OS SEGUNDOS ATÉ O USUARIO RESPONDER E RESULTADO DA SOMA
function respostaHumano(x, y) {
    ...
    diferencaTempo2 = formatarValor(diferencaTempo2);

    console.log(`Seu tempo foi ${diferencaTempo2} segundos!`);
...
console.log();
console.log('Espero que você se dedique aos estudos. Para isso uma boa noite de sono e uma boa alimentação irão te ajudar no processo de estudar!')
console.log('REGRAS DO JOGO:\n')
console.log('A cada semana você irá responder a uma soma, no total são 3 semanas onde é feito a média e se a média for maior que 7 você passa para o modulo 2.');
console.log('Para ir bem nas respostas não basta apenas responder correto, tem que responder rapido. Cada 2 segundos que passa é 1 ponto que se perde. ');
...
do{
    ...
    if(pronto == 1){
        ...
        do{
            ...
            //ENQUANTO MENOR QUE 3 MODULOS, EXECUTA!
            while(modulo < 3){
                console.log('--------------------------------------');
                console.log();
                ...
                if(fazendoCorrecao == 1){
                    ...
                    }else if(resultadoHumano[1] > 4000 &&  resultadoHumano[1] <= 8000){
                        jogador.estudar1(1);
                        console.log(`Parabens, voce acertou! Sua nota foi ${jogador.estudar}`);
                    }else if(resultadoHumano[1] > 8000 && resultadoHumano[1] <= 10000){
                        jogador.estudar1(2);
                        console.log(`Parabens, voce acertou! Sua nota foi ${jogador.estudar}`);
                    }else if(resultadoHumano[1] > 10000 && resultadoHumano[1] <= 12000){
                        jogador.estudar1(3);
                        console.log(`Parabens, voce acertou! Sua nota foi ${jogador.estudar}`);
                    }else if(resultadoHumano[1] > 12000 && resultadoHumano[1] <= 14000){
                        jogador.estudar1(4);
                        console.log(`Parabens, voce acertou! Sua nota foi ${jogador.estudar}`);
                    }else if(resultadoHumano[1] > 14000 && resultadoHumano[1] <= 16000){
                        jogador.estudar1(5);
                        console.log(`Parabens, voce acertou! Sua nota foi ${jogador.estudar}`);
                    }else{
                        jogador.estudar1(6);
                        console.log(`Parabens, voce acertou! Porem foi muito lento... Sua nota foi ${jogador.estudar}`) ;
                    }
    ...
    jogarNovamente = +prompt('Deseja jogar novamente: 1(SIM) 0(NÃO)');
